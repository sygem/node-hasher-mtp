...
#undef TEST_ADLEN
    ...
    bool verify_tcr(const char *input,
                    uint32_t nonce,
                    const uint8_t hash_root_mtp[16],
                    const uint64_t block_mtp[L_TCR * 2][128],
                    const std::deque<std::vector<uint8_t>> proof_mtp[L_TCR * 3],
                    uint256 *hash_out) {

        MerkleTree::Buffer const root(&hash_root_mtp[0], &hash_root_mtp[16]);
        block blocks[L_TCR * 2];

        for (int i = 0; i < (L_TCR * 2); ++i) {
            memcpy(blocks[i].v, block_mtp[i],
                   sizeof(uint64_t) * ARGON2_QWORDS_IN_BLOCK);
        }
